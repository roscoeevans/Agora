-- Migration: 018_search_users_function
-- Description: Sophisticated user search function with popularity + recency ranking
-- Author: AI Assistant
-- Date: 2025-10-28
...
create or replace function public.search_users_v1(
  ...
  after_handle text default null  -- cursor: last handle returned; simple & stable
)
returns table (
  user_id uuid,
  handle text,
...
-- =============================================================================
-- SUGGESTED CREATORS FUNCTION
-- =============================================================================

-- Function: Get suggested creators (popular, active users)
...
-- =============================================================================
-- HELPER: EXACT HANDLE LOOKUP
-- =============================================================================

-- Function: Fast exact handle lookup (useful for @mentions)
create or replace function public.lookup_user_by_handle(
  handle_query text
)
returns table (
  user_id uuid,
...
as $$
  select
    ...
    u.followers_count
  from public.users u
  where u.handle = lower(regexp_replace(trim(handle_query), '^@', ''))
...
comment on function public.search_users_v1(text, uuid, int, text) is 
  'Main user search function with popularity-blended ranking. Returns users matching query with relevance + popularity scores.';

comment on function public.suggested_creators(uuid, int) is 
  'Returns popular, active users not followed by viewer, sorted by popularity.';

comment on function public.lookup_user_by_handle(text) is 
  'Fast exact handle lookup for @mentions and profile navigation.';

-- =============================================================================
-- GRANTS (allow authenticated users to call search)
-- =============================================================================

grant execute on function public.search_users_v1(text, uuid, int, text) to authenticated;
grant execute on function public.suggested_creators(uuid, int) to authenticated;
