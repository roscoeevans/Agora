...
public final class AuthManager: NSObject, AuthServiceProtocol, @unchecked Sendable {
            ...
            let delegate = SignInDelegate { result in
                Task { @MainActor in
                    ...
                    case .success(let authorization):
                        do {
                            let authResult = try await self.handleSuccessfulSignIn(authorization)
    ...
    public func refreshToken() async throws -> String {
        ...
        guard let token = await sessionStore.accessToken else {
            throw AuthError.refreshFailed
        }
    ...
    /// Initiates Sign in with Apple flow (legacy method)
    @MainActor
    public func signInWithAppleLegacy() async throws {
    ...
    @MainActor
    private func handleSuccessfulSignIn(_ authorization: ASAuthorization) async throws -> AuthResult {
        ...
        return AuthResult(
            ...
            expiresAt: nil // Apple tokens don't have explicit expiration
        )
    }
}
...
private class SignInDelegate: NSObject, ASAuthorizationControllerDelegate {
    ...
    nonisolated init(completion: @escaping @Sendable (Result<ASAuthorization, Error>) -> Void) {
